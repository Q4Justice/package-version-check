...
async function run() {
    try {
        ...
        const curVersion = Number(JSON.parse(Buffer.from(curPackageJson, 'base64').toString()).version);
        ...
        const prevVersion = Number(JSON.parse(Buffer.from(prevPackageJson, 'base64').toString()).version);
